...
// min/max value setup
  float vmin = config_.getFloat("minvalue", missing);  // get from yaml file
  float vmax = config_.getFloat("maxvalue", missing);  // get from yaml file
...
// the lines above are not needed because we would prefer ratio be positive and never negative

// check if threshold should be absolute or not    // added this and line below
...
//  Check if difference is within min/max value range and set flag  // orig
//  Check if ratio is within min/max value range and set flag
//        float diff = val[jobs] - ref[jobs];  //  dont need anymore
        float ratio = val[jobs]/ref[jobs];  //   defines ratio
        if (absval) {
          ratio = fabs(ratio);
        }
        for (size_t jv = 0; jv < filtervars.nvars(); ++jv) {
//          if (vmin != missing && diff < vmin) flagged[jv][jobs] = true;  // orig
          if (vmin != missing && ratio < vmin) flagged[jv][jobs] = true;
//          if (vmax != missing && diff > vmax) flagged[jv][jobs] = true;  // orig
