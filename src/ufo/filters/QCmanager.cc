...
void QCmanager::print(std::ostream & os) const {
  for (size_t jj = 0; jj < observed_.size(); ++jj) {
    ...
    for (size_t jobs = 0; jobs < iobs; ++jobs) {
      ...
      if ((*flags_)[jj][jobs] == QCflags::ratioref) ++iratioref;  //  added
    }
    ...
    if (obsdb_.isDistributed()) {
      ...
      obsdb_.comm().allReduceInPlace(iratioref,   eckit::mpi::sum());  //  added
    }
...
//  added iratioref to line above
//           ignss + idiffref + iseaice + itrack + ibuddy + idydx == iobs);  //  orig
  }
}
